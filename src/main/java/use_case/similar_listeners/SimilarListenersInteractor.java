...
public class SimilarListenersInteractor implements SimilarListenersInputBoundary {
    ...
    public void execute(SimilarListenersInputData similarListenersInputData) {
        ...
        if (userDataAccessObject.getFollowedArtists().isEmpty()) {
            similarListenersPresenter.prepareFailView("You do not follow any artists. Similar Listeners cannot be determined.");
        }
        else {
            ...
            final SimilarListenersOutputData similarListenersOutputData = new SimilarListenersOutputData(followedArtists, false);
